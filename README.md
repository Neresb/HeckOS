...
Você acaba de instalar o HeckOS. Em alguns minutos, sua empresa vai
...
```
Clona o https://github.com/Neresb/HeckOS.git na pasta atual,
...
```
git clone https://github.com/Neresb/HeckOS.git
cd HeckOS
code .
```
...
Quando o `/instalar` terminar, renomeia a pasta `HeckOS/` pro nome do teu
negócio (fecha o VS Code, renomeia no Explorer/Finder, abre de novo). A
pasta não fica como "HeckOS" — ela é o teu negócio agora.
...
closed loop dentro do HeckOS (decide → executa → captura → realimenta →
ajusta sozinho).
