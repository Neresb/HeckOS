...
name: instalar
description: >
  Instala o HeckOS no negócio do usuário. Entrevista sobre empresa, tom de voz,
  ...
  pedir explicitamente "rodar /instalar", "instalar o HeckOS", "primeiro setup".
---

# /instalar — Instalação inicial do HeckOS
...
Conferir o nome da pasta atual (`basename "$(pwd)"`). Se for `mazyos`, `HeckOS`, `HeckOS-main`, `mazyos-main` ou variação genérica:

> "Notei que a pasta atual ainda tem nome genérico ('<nome-atual>'). O ideal é a pasta ter o nome do seu negócio, não 'HeckOS'. Quando terminarmos o setup, te lembro de renomear (é rápido — fechar VS Code, renomear a pasta no Finder/Explorer, abrir de novo). Bora seguir?"
